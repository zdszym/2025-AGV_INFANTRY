...
#include "can.h"
#endif
//extern power_Calc_Data_t power_calc_data[4]; //用来从其他舵轮的数据计算总功率
int bsp_flag=0;

//can的发送和接受在一起，接受到数据后，调用回调函数,电机和编码器是这样
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
	...
	if (hcan == &hcan2)
	{
        ...
        if(CAN_RxHeaderStruct.StdId==steering_wheel.parameter.CANID)
        {
            steering_wheel_feedback_handler(&steering_wheel, rxdata);
        }
		//接受其他舵轮轮组的速度和扭矩
		switch(CAN_RxHeaderStruct.StdId){
			case 0x02A:
				wheel_data_unpack(&wheel_data[0], rxdata);
				break;
			case 0x02B:
				wheel_data_unpack(&wheel_data[1], rxdata);
				break;
			case 0x02C:
			
				wheel_data_unpack(&wheel_data[2], rxdata);
				break;
			
			case 0x02D:
				wheel_data_unpack(&wheel_data[3], rxdata);
				break;
			case 0x01E://接受功率限制
			    limit_power_data_unpack(&chassis_power_control,rxdata);
