...
#include "math.h"
    ...
    #include "can.h"
#endif
WheelComm_t wheel_communication[4];
/* CAN Bus Settings ----------------------------------------------------------*/
#if defined(STM32F105) || (STM32F407)
...
//QAQ写好烂，这边是新通信相关的一部分函数

//数据处理
void steering_wheel_dataprocess(steering_wheel_t *_steering_wheel,uint8_t rx_data[8]){
	float velocity=0;
	float theta=0;
     memcpy(&_steering_wheel->velocity_x, &rx_data[0], 4);
...
//新通信处理底盘c板会传消息
void steering_wheel_feedback_handler(steering_wheel_t *_steering_wheel, uint8_t data[8])
{
    //TODO:处理数据
    steering_wheel_dataprocess(_steering_wheel,	data);
