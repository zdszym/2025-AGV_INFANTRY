...
Steering_Wheel_MotorCommandUpdate(steering_wheel_t *steering_wheel)
{
	...
	if (Steering_CheckHandleLegitimacy(steering_wheel) == STEERING_WHEEL_OK)
	{
		...
		calculate_true_power(wheel_data,&chassis_power_control);
		...
		calculate_torque_current_according_to_scaled_power(chassis_power_control.scaled_motion_part_power_32);
#endif
#if defined(DIRECTIVE_MOTOR_M3508)
...
//calculate_torque_current_according_to_scaled_power(chassis_power_control.scaled_directive_part_power_32);
